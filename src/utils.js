...
const getCanData = (parsedData, ix) => {
  ...
  return {
    ...
    ignitionKey: parsedData[ix + 3]
      ? parseCanData(parsedData[ix + 3], "ignitionKey")
      : null,
    totalDistance: parsedData[ix + 4]
      ? parseCanData(parsedData[ix + 4], "totalDistance")
      : null,
    totalDistanceUnit: parsedData[ix + 4]
      ? parsedData[ix + 4].slice(0, 1) === "H"
        ? "km"
        : "I"
      : null,
    ...
    engineCoolantTemp: parsedData[ix + 8]
      ? parseInt(parsedData[ix + 8], 10)
      : null,
    fuelConsumption: parsedData[ix + 9]
      ? parseCanData(parsedData[ix + 9], "fuelConsumption")
      : null,
    fuelLevel: parsedData[ix + 10]
      ? parseFloat(parsedData[ix + 10].slice(1))
      : null,
    fuelLevelUnit: parsedData[ix + 10]
      ? parsedData[ix + 10].slice(0, 1) === "P"
        ? "%"
        : "L"
      : null,
    range: parsedData[ix + 11]
      ? parseCanData(parsedData[ix + 11], "range")
      : null,
    acceleratorPressure: parsedData[ix + 12]
      ? parseFloat(parsedData[ix + 12])
      : null,
    ...
    canExpanded: {
      canReportExpansionMask: {
        ...
        engineBrakingFactor: canExpansionMask
          ...
          : null,
        analogInputValue: canExpansionMask ? canExpansionMask[7] === "1" : null,
      ...
      adBlueLevel: parsedData[ix + 25]
        ? parseCanData(parsedData[ix + 25], "adBlueLevel")
        : null,
      adBlueLevelUnit: parsedData[ix + 25]
        ? (parsedData[ix + 25].slice(0, 1) === "P" ? "%" : "L")
          ? "L"
          : "%"
        : null,
      ...
      tachographOverspeedSignal: parsedData[ix + 29]
        ? parsedData[ix + 29] === "1"
        : null,
      tachographVehicleMotionSignal: parsedData[ix + 30]
        ? parsedData[ix + 30] === "1"
        : null,
      tachographDrivingDirection: parsedData[ix + 31]
        ? parseCanData(parsedData[ix + 31], "tachographDrivingDirection")
        : null,
      analogInputValue: parsedData[ix + 32]
        ? parseFloat(parsedData[ix + 32]) * 1000
        : null,
      engineBrakingFactor: parsedData[ix + 33]
        ? parseInt(parsedData[ix + 33])
        : null,
      pedalBrakingFactor: parsedData[ix + 34]
        ? parseInt(parsedData[ix + 34])
        : null,
      totalAcceleratorKickDown: parsedData[ix + 35]
        ? parseInt(parsedData[ix + 35])
        : null,
      totalEffectiveEngineSpeedTime: parsedData[ix + 36]
        ? parseFloat(parsedData[ix + 36])
        : null,
      totalCruiseControlTime: parsedData[ix + 37]
        ? parseFloat(parsedData[ix + 37])
        : null,
      totalAcceleratorKickDownTime: parsedData[ix + 38]
        ? parseFloat(parsedData[ix + 38])
        : null,
      totalBrakeApplications: parsedData[ix + 39]
        ? parseInt(parsedData[ix + 39])
        : null,
      ...
      expansionInformation: {
        ...
        brakeFluidLowIndicator: expansionBin ? expansionBin[1] === "1" : null,
        coolantLevelLowIndicator: expansionBin ? expansionBin[2] === "1" : null,
        ...
        serviceCallIndicator: expansionBin ? expansionBin[11] === "1" : null,
      ...
      rapidAccelerations: parsedData[ix + 47]
        ? parseInt(parsedData[ix + 47])
        : null,
      engineTorque: parsedData[ix + 48]
        ? parseFloat(parsedData[ix + 48])
        : null,
    },
  };
};
...
const getAlarm = (command, report, extra = false) => {
  ...
  if (
    ...
    const driverID =
      report.split(",")[0] !== null ? report.split(",")[0] : null;
    return {
